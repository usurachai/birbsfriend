...
contract Birdstest is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public whitelistEndDateVIP = 1658658352;
  uint256 public whitelistEndDate = 1658622352;
  uint256 public whitelistEndDateFriend = 1658672752;
    ...
    if(block.timestamp < whitelistEndDateVIP) {
      ...
      if(totalSupply() + mintAmount <= sapLimit){
        if(!isValidMerkleProof(merkleProof, whitelistSupporter) && 
          ...
          revert("This is Birbs friend round! Please wait for the next round");
        }
      }else{
        require(isValidMerkleProof(merkleProof, whitelistFriend), "This is Birbs friend round! Please wait for the next round");
      }
    }
